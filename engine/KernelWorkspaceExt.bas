...
' =====================================================================
' GetTabCategory -- BUG-191 helper
'
' Returns the Category column value from tab_registry for a given tab.
' Used by KernelFormulaWriter to decide whether to clear Output-category
' tabs even in preserve mode (to prevent stale legacy cells surviving a
' CSV layout change).
' =====================================================================
Public Function GetTabCategory(tabName As String) As String
    GetTabCategory = ""
    ...
    Dim tabSecStart As Long
    tabSecStart = KernelConfigLoader.FindSectionStart(wsCfg, CFG_MARKER_TAB_REGISTRY)
    If tabSecStart = 0 Then Exit Function
    Dim dr As Long: dr = tabSecStart + 2
    Do While Len(Trim(CStr(wsCfg.Cells(dr, TREG_COL_TABNAME).Value))) > 0
        If StrComp(CStr(wsCfg.Cells(dr, TREG_COL_TABNAME).Value), tabName, vbTextCompare) = 0 Then
            GetTabCategory = Trim(CStr(wsCfg.Cells(dr, 3).Value))
            Exit Function
        End If
        dr = dr + 1
    Loop
...
' =====================================================================
' CapturePreservedCellsInMemory -- BUG-191 helper
'
' Walks preserved_cells_config for a given tab, reads current values of
' non-formula cells, returns a Dictionary of (addressA1 -> value).
' Paired with RestorePreservedCellsInMemory to survive a ClearContents
' on Output-category tabs during RefreshFormulaTabs preserve mode.
' =====================================================================
Public Function CapturePreservedCellsInMemory(tabName As String) As Object
    Dim dict As Object
    Set dict = CreateObject("Scripting.Dictionary")
    Set CapturePreservedCellsInMemory = dict

    On Error GoTo CapErr
    Dim cfgPath As String
    cfgPath = BuildPath(KernelSnapshot.GetProjectRoot(), "config")
    cfgPath = BuildPath(cfgPath, CFG_PRESERVED_CELLS)
    If Dir(cfgPath) = "" Then Exit Function
    If Not TabExists(tabName) Then Exit Function
    ...
    For i = 1 To UBound(lines)
        Dim ln As String: ln = Trim$(lines(i))
        If Len(ln) = 0 Then GoTo NextCapLine
        Dim f() As String
        f = ParseCsvLineSimple(ln)
        If UBound(f) < 1 Then GoTo NextCapLine
        If StrComp(f(0), tabName, vbTextCompare) <> 0 Then GoTo NextCapLine
        Dim addr As String: addr = f(1)
        Dim rng As Range
        On Error Resume Next
        Set rng = ws.Range(addr)
        On Error GoTo CapErr
        If rng Is Nothing Then GoTo NextCapLine
        Dim cell As Range
        For Each cell In rng.Cells
            If cell.HasFormula Then GoTo NextCapCell
            Dim addrStr As String: addrStr = cell.Address(False, False)
            dict(addrStr) = cell.Value
NextCapCell:
        Next cell
NextCapLine:
    Next i
    Exit Function
CapErr:
    ' Return partial dict on error (non-fatal)
End Function

' =====================================================================
' RestorePreservedCellsInMemory -- BUG-191 helper
'
' Writes captured values back into cells after a tab has been cleared
' and rebuilt. Force NumberFormat = "@" for values starting with formula
' characters (AP-07/AP-50 safety).
' =====================================================================
Public Sub RestorePreservedCellsInMemory(tabName As String, dict As Object)
    If dict Is Nothing Then Exit Sub
    If dict.Count = 0 Then Exit Sub
    If Not TabExists(tabName) Then Exit Sub
    ...
    For Each key In dict.Keys
        ...
        Dim valStr As String: valStr = CStr(val)
        If Len(valStr) > 0 Then
            Dim first As String: first = Left$(valStr, 1)
            If first = "=" Or first = "+" Or first = "-" Or first = "@" Then
                ws.Range(CStr(key)).NumberFormat = "@"
            End If
        End If
        ws.Range(CStr(key)).Value = val
    Next key
End Sub
